...
async function run() {
  try {
    ...
    app.get("/dev_sprint_view", function(req, res) {
      // Render the main_page.html or any other page you want to show
      //res.render("main_page");
      //res.sendFile(path.join(__dirname, "/views/main_page.html"));
      const sprintIndex = req.query.sprintIndex;
      res.render("dev_sprint_view", {columns:sprints[sprintIndex].columns, sprintIndex: sprintIndex, sprintName: sprints[sprintIndex].name});
    });
